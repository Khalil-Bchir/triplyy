...
export const clearAuthCookies = (c: any) => {
  const opts = env.NODE_ENV === 'production' ? { path: '/', domain: env.COOKIE_DOMAIN } : { path: '/' }
  deleteCookie(c, AUTH_COOKIE_NAMES.accessToken, opts)
  deleteCookie(c, AUTH_COOKIE_NAMES.refreshToken, opts)
}
