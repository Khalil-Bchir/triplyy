...
export function resolveCorsOrigin(origin: string | null | undefined): string | null {
  ...
  // When using credentials=true, we must not respond with '*' for browser requests.
  // Echo the request Origin when present; fall back to '*' for non-browser/no-origin requests.
  return origin ?? '*'
}
